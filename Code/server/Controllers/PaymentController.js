...
const UpdatePaiment = asyncHandler(async (req, res) => {
    ...
    // search for ID appartement TO send in req Add Paiment
    const SearchAppartement = await AppartementModel.findOne({ Number_Appartement });
    console.log(Number_Appartement);
...
const GetSinglePayment = asyncHandler(async (req, res) => {
    const paiment = await Paiment.findById(req.params.id).populate("Number_Appartement").populate("cin")
    if (paiment) {
